...
def get_dataset(root, annFile, subset_size=12000):
    ...
    selected_img_ids = random.sample(all_img_ids, min(subset_size, len(coco.imgs)))
...
def train(rank, world_size, args):
    ...
    optimizer = torch.optim.SGD([p for p in fsdp_model.parameters() if p.requires_grad],
                                lr=args['lr'], momentum=0.9, weight_decay=0.0005)

    if rank == 0 and not os.path.exists("results_fsdp.csv"):
        with open("results_fsdp.csv", "w") as f:
    ...
    for epoch in range(args['epochs']):
        ...
        for images, targets in dataloader:
            ...
            loss_dict = fsdp_model(images, targets)
        ...
        if rank == 0:
            ...
            with open("results_fsdp.csv", "a") as f:
            ...
            with redirect_stdout(f_output):
                mAP = evaluate_model(fsdp_model, val_loader, device, args['val_ann'])
...
if __name__ == "__main__":
    run_training(world_size=4)  # Adjust the number of GPUs as needed
